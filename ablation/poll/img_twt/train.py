...
def train(opt):
    ...
    for epoch in range(0, epochs):
        ...
        for images, twts, caps, attrs, labels in tqdm.tqdm(train_loader, total=len(train_loader)):
            ...
            predicted_labels = [
                0 if Counter(pred[i] for pred in [pre_1, pre_2, pre_3]).most_common(1)[0][
...
def test(model, test_loader):
    ...
    for images, twts, caps, attrs, labels in tqdm.tqdm(test_loader,
        ...
        pre_1, pre_2, pre_3 = [[1 if prob >= 0.5 else 0 for prob in
                                o.detach().cpu().numpy()] for o in [o1, o2, o3]]
        predicted_labels = [
            0 if Counter(pred[i] for pred in [pre_1, pre_2, pre_3]).most_common(1)[0][0] == 0 else 1 for i in range(len(pre_1))]
        gts = np.hstack((gts, labels.cpu().numpy()))
        preds = np.hstack((preds, predicted_labels))
