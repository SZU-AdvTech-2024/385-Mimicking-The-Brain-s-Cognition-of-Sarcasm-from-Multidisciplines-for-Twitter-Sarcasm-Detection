...
def train(opt):
    ...
    for epoch in range(0, epochs):
        ...
        for images, twts, caps, attrs, labels in tqdm.tqdm(train_loader, total=len(train_loader)):
            ...
            o1, o2, o3 = model(images, twts, caps, attrs)  # 前向传播
            loss = 0
            for o in [o1, o2, o3]:
            ...
            optimizer.step()  # 更新参数
            losses += loss
            # pre_1, pre_2, pre_3 = [[1 if prob >= 0.5 else 0 for prob in
            #                     o.detach().cpu().numpy()] for o in [o1, o2, o3]]
            _, pred1 = torch.max(o1, 1)
            _, pred2 = torch.max(o2, 1)
            _, pred3 = torch.max(o3, 1)
            ...
            predicted_labels = [
                0 if Counter(pred[i] for pred in [pred1, pred2, pred3]).most_common(1)[0][
                         0] == 0 else 1 for i in range(len(pred1))]
...
def test(model, test_loader):
    ...
    for images, twts, caps, attrs, labels in tqdm.tqdm(test_loader,
        ...
        #                         o.detach().cpu().numpy()] for o in [o1, o2, o3]]
        predicted_labels = [
            0 if Counter(pred[i] for pred in [pred1, pred2, pred3]).most_common(1)[0][0] == 0 else 1 for i in range(len(pred1))]
        gts = np.hstack((gts, labels.cpu().numpy()))
        preds = np.hstack((preds, predicted_labels))
