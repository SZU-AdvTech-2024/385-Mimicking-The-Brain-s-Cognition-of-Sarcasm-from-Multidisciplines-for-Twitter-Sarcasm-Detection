...
class MyModel(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.classifier_a = Classifier(out_channels, 2)
        self.classifier_b = Classifier(out_channels, 2)
        self.classifier_c = Classifier(out_channels, 2)
    ...
    def forward(self, images, twts, caps, attrs):
        ...
        v1, v2, v3, v4 = self.sa1(img_feats), self.sa1(twt_feats), self.sa1(caps_feats), self.sa1(attrs_feats)
        v_a, v_b, v_c = self.fusion(v1, v2, v3, v4)
        return self.classifier_a(v_a), self.classifier_b(v_b), self.classifier_c(v_c)
