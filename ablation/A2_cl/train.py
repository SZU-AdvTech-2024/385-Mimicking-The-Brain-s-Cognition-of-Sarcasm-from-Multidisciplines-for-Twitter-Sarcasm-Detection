...
import sys
sys.path.append('/home/p/Documents/Codes/tweeter_sarcasm_detect/')
...
def train(opt):
    ...
    for epoch in range(0, epochs):
        ...
        for images, twts, caps, attrs, labels in tqdm.tqdm(train_loader, total=len(train_loader)):
            ...
            v1, v2, o1 = model(images, twts, caps, attrs)  # 前向传播
            loss = 0

            for o in [o1]:
                loss += criterion(v1, v2, o.float(), labels.float())  # 计算损失
            # loss /= 3
            loss.backward()  # 反向传播计算梯度
            optimizer.step()  # 更新参数
            losses += loss

            predicted_labels = output_to_labels(o1)
        ...
        info = f'**train** epoch {epoch}, train_acc: {train_acc}, train_loss: {losses.item()}, '\
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cl-weight', type=float, default=0.2, help='contrastive loss weight')
    parser.add_argument('--bce-weight', type=float, default=0.8, help='binary cross entropy loss weight')
    ...
    info = f'-------------------------------hyperparameters---------------------------\n'\
         f'lr: {opt.lr}, temp: {opt.temp}, cl-weight: {opt.cl_weight}, bce-weight:{opt.bce_weight} batch-size: {opt.batch_size}, \n'\
         '-------------------------------------------------------------------------'
    logger.info(info)
    print(info)
    train(opt)